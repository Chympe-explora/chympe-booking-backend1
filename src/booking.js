...
export function corsHeaders(env) {
  return {
    ...
    // "authorization" is required here — the admin/guide dashboards send
    // "Authorization: Bearer <token>" on every request after login,
    // and a browser's CORS preflight silently blocks any request whose
    // header isn't explicitly allowed here.
...
export async function handleSubmit(request, env, ctx) {
  ...
  // A guide linked only through the web dashboard (never redeemed a
  // Telegram code — see guides.js#redeemGuideCodeWeb) has no chatId to
  // send an actionable Telegram message to. They still get the
  // assignment (see assignBookingToGuide below) and will see it —
  // with working Confirm/Reject — in their web dashboard instead;
  // Telegram is only the storage here, same as everywhere else.
  if (assignedGuide && assignedGuide.chatId) {
  ...
  } else if (assignedGuide) {
    primaryRes = { ok: true, result: null };
    groupInfoText = `\u2139\ufe0f <b>New booking</b> — assigned to <b>${escapeHtml(assignedGuide.name)}</b> (web dashboard) for confirmation.\n\n${text}`;
  } else {
  ...
  if (actionableChatId && primaryRes.result && primaryRes.result.message_id) {
  ...
  async function sendInfoAndPersist() {
    ...
    await env.BOOKINGS.put(
      `booking:${bookingId}`,
      JSON.stringify({ sessionId, siteId, data, text, assignedGuideId: assignedGuide ? assignedGuide.id : null, receipt: receipt || null, createdAt: Date.now() }),
    ...
    // Small capped index so the web admin dashboard's Bookings feed can
    // list recent bookings without ever reading Telegram — same idea as
    // conversations.js's conv:index for the chat inbox.
    try {
      const list = JSON.parse((await env.BOOKINGS.get("recentbookings")) || "[]");
      list.push(bookingId);
      await env.BOOKINGS.put("recentbookings", JSON.stringify(list.slice(-200)));
    } catch (e) {}
  }
...
// Shared by the Telegram Confirm/Reject buttons (handleBookingCallback,
// below) AND the web admin/guide dashboard's Confirm/Reject buttons
// (see admin-api.js) — one place that actually settles a booking, so
// both surfaces behave identically and stay in sync with each other.
// `override` lets the Telegram path pass the exact message content it
// already has on hand (from the incoming callback update) instead of
// the stored copy, preserving its previous byte-for-byte behavior.
export async function setBookingStatus(env, bookingId, newStatus, override = {}) {
  ...
  if (msgRefs.length) {
    ...
    const isMedia = override.isMedia ?? false;
    const originalCaption = override.originalCaption ?? "";
    const originalText = override.originalText ?? (booking ? booking.text || "" : "");
  ...
  if (newStatus === "cancelled" && booking && booking.sessionId) {
...
export async function handleBookingCallback(cb, env) {
  ...
  if ((action === "confirm" || action === "cancel") && bookingId) {
    ...
    // Answer the callback immediately, so the guide's own Telegram
    ...
    // different chat.
    await setBookingStatus(env, bookingId, newStatus, {
      isMedia: !!(cb.message && (cb.message.photo || cb.message.document)),
      originalCaption: cb.message ? cb.message.caption || "" : "",
      originalText: cb.message ? cb.message.text || "" : "",
    });
  }
